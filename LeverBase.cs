...
public class LeverBase : MonoBehaviour
{
    ...
    public bool TryAttachLever(LeverHandle leverInHand)
    {
        ...
        if (leverInHand.TryAttachToBase(this))
        {
            ...
            // Garantir que o ManualInteractionSystem da alavanca seja notificado
            ManualInteractionSystem leverManualSystem = leverInHand.GetComponent<ManualInteractionSystem>();
            if (leverManualSystem != null)
            {
                leverManualSystem.OnObjectPickedUp(); // Marca como "pego" para esconder o ícone
            }
            
            // MANTER o ícone da base ativo e mudar o texto para "usar alavanca"
            ManualInteractionSystem baseManualSystem = GetComponent<ManualInteractionSystem>();
            if (baseManualSystem != null)
            {
                baseManualSystem.actionText = activateText;
                baseManualSystem.ForceShowIcon(true); // Garantir que o ícone da base está ativo
            }
    ...
    public void DetachLever()
    {
        ...
        // Notificar o ManualInteractionSystem da alavanca que ela foi largada
        ManualInteractionSystem leverManualSystem = attachedLever.GetComponent<ManualInteractionSystem>();
        if (leverManualSystem != null)
        {
            leverManualSystem.OnObjectDropped(); // Marca como "largado" para mostrar o ícone novamente
        }
        ...
        if (baseManualSystem != null)
        {
            baseManualSystem.actionText = attachText;
        }
